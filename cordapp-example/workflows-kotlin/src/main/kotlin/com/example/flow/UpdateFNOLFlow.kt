...
object UpdateFNOLFlow {
    ...
    class Initiator(val firstName: String,
            ...
            val otherCarrier: Party, val linearId: UUID
        ...
        override fun call(): SignedTransaction {
            ...
            // Create Query Criteria to find state in vault based on linear id
            val uuid: UniqueIdentifier = UniqueIdentifier(null,linearId)
            val queryCriteria = QueryCriteria.LinearStateQueryCriteria(linearId = listOf(uuid))
            val queriedStates = serviceHub.vaultService.queryBy<FNOLState>(queryCriteria).states

            // Error when query flow returns no state
            if (queriedStates.isEmpty()) {
                FlowException(message = "No matching state found")
            }

            var oldState = queriedStates
            ...
            val updatedFNOL = FNOLState(firstName, lastName, policyNum, insuredLP, lossEventLocation, firstLossEventDescription,
                    dateOfLoss, otherFirstName, otherLastName, otherLicensePlate, otherPolicy,
                    otherLossEventDescription, me, otherCarrier)

            val txCommand = Command(FNOLContract.Commands.Update(), updatedFNOL.participants.map { it.owningKey })
            val txBuilder = TransactionBuilder(notary)
                    .addInputState(queriedStates.single())
                    .addOutputState(updatedFNOL, FNOLContract.ID)
